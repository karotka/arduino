...
void UTFT_Buttons::drawButton(int buttonID) {
	...
	if (buttons[buttonID].flags & BUTTON_BITMAP) {
		...
		if (!(buttons[buttonID].flags & BUTTON_NO_BORDER))
		{
			if ((buttons[buttonID].flags & BUTTON_DISABLED)) {
        		_UTFT->setBackColor(_color_text_inactive);
	...
	else
	{
        ...
        // fixme not working
		if (buttons[buttonID].flags & BUTTON_DISABLED) {
			_UTFT->setColor(_color_back_inactive);
		} else {
			_UTFT->setColor(_color_background);
    		_UTFT->setBackColor(_color_back_inactive);
            if (buttons[buttonID].bgColor == _color_background) {
                _UTFT->setColor(_color_background);
            } else {
                _UTFT->setColor(buttons[buttonID].bgColor);
            }
        }
		...
		if (buttons[buttonID].flags & BUTTON_DISABLED) {
            ...
            if (buttons[buttonID].bgColor == _color_background) {
            	...
            	_UTFT->setBackColor(buttons[buttonID].bgColor);
            }
        }
...
void UTFT_Buttons::setButtonColors(word atxt, word iatxt, word brd,
                                   word brdhi, word back, word iback) {
	_color_text				= atxt;
	_color_text_inactive	= iatxt;
	_color_background		= back;
	_color_border			= brd;
	_color_hilite			= brdhi;
	_color_back_inactive	= iback;
}
